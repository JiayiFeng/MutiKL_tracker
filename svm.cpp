                   ...
                   int d_proj, double lambda_, double obj_threshold_, double diff_threshold_) 
{
  ...
  if(num_kernels>1)
    ...
    {
      if(d_regularizer == L2 || (d_regularizer == LPS && L_p == 2))
        QPQPSolver(si);
      else if(d_regularizer == LPS)
        LPQPSolver(si);
      else
        EntSolver(si);
    }
...
void Solver::QPQPSolver(SolutionInfo* si)
{
  ...
  g_from_Qalpha();
	...
	std::vector<double> qq(num_kernels,0);
	std::vector<double> rr(num_kernels,0);
...
void Solver::LPQPSolver(SolutionInfo* si)
{
  ...
  g_from_Qalpha();
    ...
    double T2=0.0;
    for(int n=0;n<num_kernels;n++)
      T1+=pow(-double(grad_d[n]/lambda), double(L_q));
        
    if(fabs(T1) > TAU)
    {
      T2=pow(double(T1), double((2 - L_q)/L_q));
      T1=pow(double(T1), double((2 - 2*L_q)/L_q));
      for(int n=0;n<num_kernels;n++)
      {
        if(-grad_d[n] < TAU)
          continue;
        double g_sq=dir_i*Qalpha_all[i][n] + dir_j*Qalpha_all[j][n];
        double mult=(2 - L_q)*T1*pow(-double(grad_d[n]/lambda), double(2*L_q-2));
        mult+=(L_q - 1)*T2*pow(-double(grad_d[n]/lambda), double(L_q-2));
        dir_hess+=(mult*g_sq*g_sq/lambda); 
      }
    }
    else
    {
    ...
    while(1)
    {
      ...
      if(fabs(T1) > TAU)
      {
        T2=pow(T1, double((2 - L_q)/L_q));
        T1=pow(T1, double((2 - 2*L_q)/L_q));
        for(int n=0;n<num_kernels;n++)
        {
          if(-grad_d[n] < TAU)
            continue;
...
void Solver::EntSolver(SolutionInfo* si)
{
  ...
  info("initial obj=%f \n", -obj);
	...
	std::vector<Qfloat> rr(num_kernels,0);
											 ...
											 proj_c *proj_fp)
{
  ...
  double grad_norm_l2=norm_l2(gradk, num_kernels);
  double pg_norm_l2=norm_l2(&proj_grad[0], num_kernels);
  double pg_norm_inf=norm_inf(&proj_grad[0], num_kernels);
														 ...
														 proj_c *proj_fp)
{
  ...
  // k-th iterate
  std::vector<double> dk(num_kernels,0);
  
  // k-th descent direction 
  std::vector<double> dirk(num_kernels,0);
  for(int n=0;n<num_kernels;n++)
  {
    dk[n]=d[n];
    dirk[n]=0;
  ...
  std::vector<double> gradk(num_kernels,0); 
  grad(&gradk[0], &obj_grad_fp[0]);
  ...
  for(int iter=1; iter <= ProjGrad::max_iter; iter++)
  {
    ...
    if(converged(&dk[0], &gradk[0], obj, obj_grad_fp, proj_fp))
      return finalize(&dk[0], obj, si);
    ...
    proj_fp->dd(&dk[0], &gradk[0], &dirk[0], d_max_idx);
													 ...
													 proj_c *proj_fp)
{
  ...
  double step0=sqrt(log(double(num_kernels)));
  ...
  for(int iter=1; iter <= ProjGrad::max_iter; iter++)
  {
    ...
    double grad_norm_inf=norm_inf(&gradk[0], num_kernels); 
...
double Solver::d_lp(void)
{
  ...
  for(int n=0;n<num_kernels;n++)
  {
    obj+=pow(-double(grad_d[n]/lambda), double(L_q));
    // info("n=%d L_q=%f theta=%f obj in d_lp=%f \n", n, L_q, -grad_d[n]/lambda, obj);
  }
  double denom=pow(obj, double((L_q - 2)/L_q));
  obj=pow(obj, double(2/L_q));
  
  for(int n=0;n<num_kernels;n++)
    d[n]=pow(fabs(-double(grad_d[n]/lambda)), double(L_q-1))/denom;
  
  return 0.5*lambda*obj;
}
...
int Solver::qpqp_select_working_set(int &out_i, int &out_j)
{
	...
	std::vector<Qfloat> QQ_i(active_size,0);
	std::vector<Qfloat> QQD(active_size,0);
...
int Solver::ent_select_working_set(int &out_i, int &out_j)
{
	...
	std::vector<Qfloat> QQ_i(active_size,0);
	std::vector<Qfloat> QQD(active_size,0);
	std::vector<Qfloat> Qalpha(active_size,0);
...
public:
  ...
  // Don't fully understand what is going on here 
	Qfloat *get_Qn(int n, int i, int len) const
	{
		Qfloat *data;
		int start, j, real_i=index[i];
    
    if((start=cache[n]->get_data(real_i,&data,len)) < len)
			{
				for(j=start;j<len;j++)
    ...
    for(int n=0;n<num_kernels;n++)
    {
      if(cache[n]->get_data(real_i,&data,l) < l)
      {
        for(int j=0;j<l;j++)
          data[j]=(Qfloat)((this->kernel_all)(n,real_i,j));
      }
...
void save_kernels(FILE *fp,const struct svm_parameter *param)
{
  ...
  if(param->d_proj == SIMPLEX)
	...
	for(int i=0;i<param->num_kernels;i++)
	{
		...
		if(param->kernels[i].kernel_type == PRECOMPUTED)
			fprintf(fp,"-t %d -w %.17g -f %s.test\n",PRECOMPUTED,param->kernels[i].coef,
			param->kernels[i].precomputed_filename);
		else
			fprintf(fp,"-t %d -w %.17g -d %d -g %.17g -r %.17g\n",param->kernels[i].kernel_type,
				param->kernels[i].coef,param->kernels[i].degree,param->kernels[i].gamma,
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param)
{
  ...
  if(param->d_regularizer != ENT &&
     ...
     param->d_regularizer != LPS)
    return "unknown regularizer";

  if(param->d_regularizer == LPS && 
     (param->L_p <= 1.0))
    return "p for LPS regularization must be > 1.0";
  ...
  if(param->d_regularizer == LPS &&
     param->d_proj != NN_ORTHANT && 
     param->solver_type == SMO)
    return "SMO solver for LPS regularizer only works with nn-orthant.";
